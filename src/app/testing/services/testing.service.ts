...
export class TestingService {
  ...
  private mockResult(cfg: TestConfig) {
    ...
    const metrics: Metrics = {
      total, detections: Math.round(det*multiplier), detectionRatio: (det*multiplier)/total,
    ...
    const sql = `/* ${cfg.mode} */
SELECT event_id, event_time, event_type, amountUSD, ${cfg.artifact==='Scorecards' ? 'score' : 'rule_action'} AS action, label AS transaction_label
FROM event_result
